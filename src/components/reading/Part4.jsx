...
export function Part4Left({ answers, setAnswer, fontScale }) {
  ...
  return (
    <div>
      ...
      <p style={{color:'#636E72',fontSize:'.88em',marginBottom:16}}>{EXAM_DATA.part4.instructions}</p>

      <div style={{background:'#fff',border:'1px solid #E2E0F0',borderRadius:12,padding:'20px 24px'}}>
        <div style={{fontFamily:'Outfit,system-ui',fontWeight:700,fontSize:'1em',marginBottom:16,color:'#2D3436'}}>{EXAM_DATA.part4.article.title}</div>
      ...
      {/* Sentence Bank */}
      <div style={{marginTop:20,background:'#F9F8FF',border:'2px dashed #B0A3E8',borderRadius:14,padding:'14px 16px'}}
        onDragOver={e=>e.preventDefault()} onDrop={dropToBank}>
        <div style={{fontSize:'.75em',fontWeight:700,color:'#6C5CE7',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:10}}>Sentence Bank — drag into gaps above</div>
          ...
          {EXAM_DATA.part4.sentences.map(s=>{
            ...
            return (
              <div key={s.id} className={`stile${isUsed?' used':''}`}
                ...
                <span style={{width:24,height:24,borderRadius:7,background:isUsed?'#A0AEC0':'#6C5CE7',color:'#fff',fontWeight:700,fontSize:'.8em',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{s.id}</span>
                <span style={{flex:1}}>{s.text}</span>
                {isUsed&&<span style={{fontSize:'.75em',color:'#00B894',fontWeight:600,whiteSpace:'nowrap'}}>✓ Gap {usedMap[s.id]}</span>}
              </div>
            );
          })}
        </div>
...
export function Part4Right({ answers }) {
  return (
    <div>
      ...
      <p style={{color:'#636E72',fontSize:'.88em',marginBottom:16}}>Drag sentences from the bank into the numbered gaps in the article.</p>
      {[1,2,3,4,5].map(n=>{
        ...
        return (
          <div key={n} className="qcard mb-3">
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:assigned?8:0}}>
              <QNumBadge n={n+15} />
              <span style={{fontWeight:600,fontSize:'.9em',color:'#2D3436'}}>Gap {n}</span>
              <span style={{marginLeft:'auto',fontSize:'.76em',fontWeight:600,padding:'2px 10px',borderRadius:99,background:assigned?'#DCFCE7':'#F1F5F9',color:assigned?'#166534':'#9CA3AF'}}>
                {assigned?`Letter ${assigned}`:'Not answered'}
              </span>
            </div>
            {sent&&<p style={{fontSize:'.82em',lineHeight:1.45,color:'#636E72',borderLeft:'3px solid #6C5CE7',paddingLeft:8,marginLeft:36}}>{sent.text}</p>}
          </div>
        );
      })}
      <div style={{background:'#EAE8FD',border:'1px solid #C4B8F3',borderRadius:12,padding:'14px 16px',marginTop:4}}>
        <div style={{fontSize:'.8em',fontWeight:700,color:'#4A3DB8',marginBottom:5}}>💡 Strategy Tip</div>
        <p style={{fontSize:'.8em',lineHeight:1.55,color:'#4A3DB8',opacity:.85}}>Look for pronouns (it, they, this), connecting phrases (however, what is more), and words that refer back to ideas already mentioned. These clues link gaps to the surrounding sentences.</p>
      </div>
    </div>
  );
}
