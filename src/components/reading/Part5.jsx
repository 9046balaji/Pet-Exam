...
export function Part5Left({ answers, setAnswer, fontScale }) {
  ...
  return (
    <div>
      ...
      <p style={{color:'#636E72',fontSize:'.88em',marginBottom:16}}>{EXAM_DATA.part5.instructions}</p>
...
export function Part5Right({ answers, setAnswer, flagged, toggleFlag }) {
  return (
    <div>
      ...
      <p style={{color:'#636E72',fontSize:'.88em',marginBottom:16}}>Select the correct word. Your choice updates the inline text on the left automatically.</p>
      {EXAM_DATA.part5.gaps.map((g, i) => {
        ...
        return (
          <div key={g.id} className="qcard fu" style={{animationDelay:`${i*55}ms`}}>
            ...
            {ans&&<button style={{marginTop:6,fontSize:'.75em',color:'#A0AEC0',background:'none',border:'none',cursor:'pointer'}} onClick={()=>setAnswer(g.id,null)}>Clear answer</button>}
          </div>
        );
      })}
    </div>
